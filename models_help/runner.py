...
import time
from models_help.build import text_question, get_images, context_description_prompt, context_description_image, context_prompt, answer_description_image, questions_description, questions_options
...
def open_ai(model, prompt, images):
    mensagens = [
        ...
        {'role' : 'assistant', 'content' : 'Responda apenas a alternativa correnta dentro dos parênteses, ex: (A)'}
    ]
    print(mensagens)
...
def send_text(model, prompt, images=None, timeout=None):
    print("Iniciando o multi processamento")
...
def test_models(questions, primary_models, secundary_models=None, predict_file=None, timeout=None):    
