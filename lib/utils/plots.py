...
import pandas as pd
import numpy as np
from lib.utils import format_time, get_predict_data, gen_modelos_str
...
def discipline_performance(models, questions, group_model: bool = False, normalize: bool = False) -> plt.Figure:    
    ...
    df = pd.DataFrame(predict_data.values())
...
def histogram(df, column, max_unique_bins=30) -> plt.Figure:
    ...
    if unique_values <= max_unique_bins:
        bins = np.arange(df[column].min(), df[column].max() + 2) - 0.5 
        xticks = np.arange(df[column].min(), df[column].max() + 1, 1) 
    else:
        bins = 'auto'  
        xticks = None 
